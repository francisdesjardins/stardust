...
- **Cache helpers** — `createCachedSlice(api, options?)` / `createCachedSlice(api, path, options?)`: the managed store field holds a **`CachedState<T>`** discriminated union (`CachedIdle | CachedPending<T> | CachedFresh<T> | CachedExpired<T> | CachedRejected<T>`), making cache status observable via `watch` and `createDerivedStore`. Use `cachedFresh(data)` as initial state — it returns `CachedState<T>` (wide union) so TypeScript infers `TSnapshot` correctly. Use `cachedFresh(data, expiresAt)` to embed a TTL directly on the data — the expiry timer is **scheduled at construction** when `expiresAt` is present; on firing the field transitions to `CachedExpired<T>` (observable!). `set(data, expiresAt?)` and `refresh(fetcher, { expiresAt? })` also accept a per-call timestamp. `startAutoRefresh({ interval })` arms fetch-on-expire and stamps `expiresAt = Date.now() + interval` on each refreshed value; `stopAutoRefresh()` sets `autoRefreshEnabled = false` — the expiry timer keeps firing (observable!) but no fetch is triggered. `refresh()` is **single-flight** — concurrent callers share one execution. `placeholder` on `CachedOptions` is the helper-level default for `CachedPending.data`; per-call `placeholder` in `CachedRefreshOptions` overrides it. `keepPreviousData` and `placeholder` are mutually exclusive (discriminated union). `refreshOnExpire` receives `(current: TValue | undefined, api)` and must return `Promise<TValue>`.
