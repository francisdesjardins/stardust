...
> **Experimental.** API may change. Not included in the main library build or barrel exports.
...
```ts
import { createStore } from '@yourorg/dialog/store';
import { useStore } from '@yourorg/dialog/src/store/solid';
...
import { shallowEqual } from '@yourorg/dialog/store';
...
Returns a signal accessor `() => TSlice`. The signal only updates when `equals(prev, next)` returns `false` (default `Object.is`). Pass `shallowEqual` from the main store barrel when your selector returns a new object literal on every call.
...
```tsx
import { useSuspenseStore } from '@yourorg/dialog/src/store/solid';
